...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate
{
    ...
    public static class DialogType
    {
        ...
        public static final int SuperGroup = 7;
        public static final int GroupsAll = 8;
    ...
    /**
     * @param currentAccount {@link Integer}
     * @param type {@link DialogType}
     * @return
     */
    public static ArrayList<TLRPC.TL_dialog> getDialogs(int currentAccount, int type)
    {
        switch (type)
        {
            case DialogType.All:
                return getInstance(currentAccount).dialogsAll;
            case DialogType.ServerOnly:
                return getInstance(currentAccount).dialogsServerOnly;
            case DialogType.GroupsOnly:
                return getInstance(currentAccount).dialogsGroupsOnly;
            case DialogType.Forward:
                return getInstance(currentAccount).dialogsForward;
            case DialogType.Users:
                return getInstance(currentAccount).dialogsUsers;
            case DialogType.Groups:
                return getInstance(currentAccount).dialogsGroups;
            case DialogType.SuperGroup:
                return getInstance(currentAccount).dialogsSuperGroups;
            case DialogType.GroupsAll:
                return getInstance(currentAccount).dialogsGroupsAll;
            case DialogType.Channels:
                return getInstance(currentAccount).dialogsChannels;
            case DialogType.Bots:
                return getInstance(currentAccount).dialogsBots;
            case DialogType.Favorites:
                return getInstance(currentAccount).dialogsFavorites;
            case DialogType.Admin:
                return getInstance(currentAccount).dialogsAdmin;
            case DialogType.Unread:
                return getInstance(currentAccount).dialogsUnread;
            case DialogType.Hidden:
                return getInstance(currentAccount).dialogsHidden;
            default:
                return getInstance(currentAccount).dialogs;
        }
    }
    //
    ...
    //
    public ArrayList<Integer> joiningToChannels = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsSuperGroups = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsFavorites = new ArrayList<>();
    ...
    public MessagesController(int num)
    {
        ...
        maxPinnedDialogsCount = FeaturedSettings.chatsSettings.maxPinnedDialogsCount; // mainPreferences.getInt("maxPinnedDialogsCount", 5);
    ...
    public void updateConfig(final TLRPC.TL_config config)
    {
        AndroidUtilities.runOnUIThread(() ->
        {
            ...
            maxPinnedDialogsCount = FeaturedSettings.chatsSettings.maxPinnedDialogsCount; // config.pinned_dialogs_count_max;
    ...
    public void cleanup()
    {
        ...
        dialogsSuperGroups.clear();
        dialogsBots.clear();
        dialogsFavorites.clear();
        dialogsHidden.clear();
        dialogsAdmin.clear();
        dialogsUnread.clear();
        //
            ...
            if (dialog != null)
            {
                ...
                if (onlyHistory == 0 || onlyHistory == 3)
                {
                    ...
                    else
                    {
                        ...
                        dialogsSuperGroups.remove(dialog);
                        dialogsBots.remove(dialog);
                        dialogsFavorites.remove(dialog);
    ...
    public void updateTimerProc()
    {
        ...
        if (!onlinePrivacy.isEmpty())
        {
            ...
            if (!toRemove.isEmpty())
            {
                ...
                Integer[] ids = toRemove.toArray(new Integer[0]);
                AndroidUtilities.runOnUIThread(() ->
                {
                    ...
                    NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.userStatusChanged, 0, ids);
                });
            }
        }
    ...
    private void updatePrintingStrings()
    {
        ...
        AndroidUtilities.runOnUIThread(() ->
        {
            ...
            if (printingStrings.size() > 0)
                NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.userTyping);
        });
    }
    ...
    {
        if (!FeaturedSettings.chatsSettings.showTyping)
            return;
                        ...
                        if (res instanceof TLRPC.TL_updates_channelDifference || res instanceof TLRPC.TL_updates_channelDifferenceEmpty)
                        {
                            if (!res.new_messages.isEmpty())
                            {
                                ...
                                MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
                                {
                                    ...
                                    {
                                        AndroidUtilities.runOnUIThread(() -> NotificationsController.getInstance(currentAccount).
                                                processNewMessages(pushMessages, true, false));
                                    }
                        ...
                        Utilities.stageQueue.postRunnable(() ->
                        {
                            if (!res.new_messages.isEmpty() || !res.new_encrypted_messages.isEmpty())
                            {
                                ...
                                MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
                                {
                                    ...
                                    {
                                        AndroidUtilities.runOnUIThread(() -> NotificationsController.getInstance(currentAccount).
                                                processNewMessages(pushMessages, !(res instanceof TLRPC.TL_updates_differenceSlice), false));
                                    }
                                    MessagesStorage.getInstance(currentAccount).putMessages(res.new_messages, true, false, false, DownloadController.getInstance(currentAccount).getAutodownloadMask());
                                });
        ...
        else if (updates instanceof TLRPC.TL_updateShortChatMessage || updates instanceof TLRPC.TL_updateShortMessage)
        {
            ...
            if (user != null && user.status != null && user.status.expires <= 0)
            {
                ...
                if (userId != UserConfig.getInstance(currentAccount).clientUserId)
                    AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.userStatusChanged, userId));
            }
    ...
    public boolean processUpdateArray(ArrayList<TLRPC.Update> updates, final ArrayList<TLRPC.User> usersArr,
                                      final ArrayList<TLRPC.Chat> chatsArr, boolean fromGetDifference)
    {
        if (updates.isEmpty())
        {
        ...
        for (int c = 0, size3 = updates.size(); c < size3; c++)
        {
            ...
            if (baseUpdate instanceof TLRPC.TL_updateNewMessage || baseUpdate instanceof TLRPC.TL_updateNewChannelMessage)
            {
                ...
                if (message.action instanceof TLRPC.TL_messageActionChatDeleteUser)
                {
                    ...
                    else if (message.from_id == UserConfig.getInstance(currentAccount).getClientUserId() &&
                            message.action.user_id == UserConfig.getInstance(currentAccount).getClientUserId())
                    {
                        continue;
                    }
                }
            ...
            else if (baseUpdate instanceof TLRPC.TL_updateUserTyping || baseUpdate instanceof TLRPC.TL_updateChatUserTyping)
            {
                ...
                if (user_id != UserConfig.getInstance(currentAccount).getClientUserId())
                {
                    ...
                    onlinePrivacy.put(user_id, ConnectionsManager.getInstance(currentAccount).getCurrentTime());
                    if (user_id != UserConfig.getInstance(currentAccount).clientUserId)
                        AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.userStatusChanged, user_id));
                }
            }
                    ...
                    else if (baseUpdate instanceof TLRPC.TL_updateUserStatus)
                    {
                        ...
                        if (currentUser != null)
                        {
                            ...
                            if (update.user_id != UserConfig.getInstance(currentAccount).getClientUserId())
                                AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.userStatusChanged, update.user_id));
                        }
    ...
    public void sortDialogs(SparseArray<TLRPC.Chat> chatsDict)
    {
        ...
        for (int a = 0; a < dialogs.size(); a++)
        {
            ...
            //
            if (Favorite.getInstance().isHidden(d.id))
            {
                dialogsHidden.add(d);
                continue;
            }
            ...
            if (lower_id != 0 && high_id != 1)
            {
                ...
                if (DialogObject.isChannel(d))
                {
                    ...
                    if (chat != null && (chat.megagroup && (chat.admin_rights != null &&
                            (chat.admin_rights.post_messages || chat.admin_rights.add_admins)) || chat.creator))
                    {
                ...
                else if (lower_id < 0)
                {
                    ...
                    dialogsGroupsAll.add(d);
                    TLRPC.Chat chat = getChat(-lower_id);
                    if (chat != null && (chat.creator || (chat.admins_enabled && chat.admin)))
                        dialogsAdmin.add(d);
                    //
                }
                //
        ...
        if (proxyDialog != null && isLeftProxyChannel)
        {
            ...
            if (FeaturedSettings.chatsSettings.showProxyAdsChannel)
                dialogsAll.add(0, proxyDialog);
        }
        if (!selfAdded)
        {
            TLRPC.User user = UserConfig.getInstance(currentAccount).getCurrentUser();
            if (user != null && !Favorite.getInstance().isHidden((long)user.id))
            {
