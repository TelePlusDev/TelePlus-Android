...
public class NotificationsController
{
    ...
    public void processNewMessages(final ArrayList<MessageObject> messageObjects, final boolean isLast, final boolean isFcm)
    {
        ...
        notificationsQueue.postRunnable(() ->
        {
            ...
            for (int a = 0; a < messageObjects.size(); a++)
            {
                ...
                {
                    int notifyOverride = getNotifyOverride(preferences, dialog_id, messageObject);
                    if (notifyOverride == -1)
                    {
            ...
            if (added && isFcm)
            {
                ...
                int notifyOverride = getNotifyOverride(preferences, dialog_id, null);
                if (notifyCheck)
                {
    ...
    public void processDialogsUpdateRead(final LongSparseArray<Integer> dialogsToUpdate)
    {
        ...
        notificationsQueue.postRunnable(() ->
        {
            ...
            for (int b = 0; b < dialogsToUpdate.size(); b++)
            {
                ...
                int notifyOverride = -1; //getNotifyOverride(preferences, dialog_id, null);
                if (notifyCheck)
                {
                    Integer override = pushDialogsOverrideMention.get(dialog_id);
                    if (override != null && override == 1)
    ...
    public void processLoadedUnreadMessages(final LongSparseArray<Integer> dialogs,
    ...
    private int getNotifyOverride(SharedPreferences preferences, long dialog_id, MessageObject messageObject)
    {
        boolean allowForUser = messageObject != null && Favorite.getInstance().isUserNotificationException(messageObject.getFromId());
        if (allowForUser)
            return -1;
    ...
    private void playInChatSound()
    {
        ...
        {
            SharedPreferences preferences = MessagesController.getNotificationsSettings(currentAccount);
            int notifyOverride = getNotifyOverride(preferences, opened_dialog_id, null);
            if (notifyOverride == 2)
            {
                return;
            }
    ...
    private void showOrUpdateNotification(boolean notifyAboutLast)
    {
        ...
        try
        {
            ...
            int notifyOverride = getNotifyOverride(preferences, override_dialog_id, null);
            boolean value;
            if (notifyOverride == -1)
            {
                value = (int) dialog_id < 0 ? preferences.getBoolean("EnableGroup", true) : preferences.getBoolean("EnableAll", true);
