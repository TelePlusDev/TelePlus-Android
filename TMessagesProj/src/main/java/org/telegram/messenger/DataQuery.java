...
public class DataQuery
{
    ...
    public void loadMusic(final long uid, final long max_id)
    {
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
        {
            ...
            AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.musicDidLoaded, uid, arrayList));
        });
    }
    //---------------- MEDIA END ----------------
    ...
    public MessageObject loadPinnedMessage(final int channelId, final int mid, boolean useQueue)
    {
        if (useQueue)
        {
            MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() -> loadPinnedMessageInternal(channelId, mid, false));
        }
        else
        {
        ...
        if ((int) dialogId == 0)
        {
            ...
            MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
            {
                try
                {
                    ...
                    AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.didLoadedReplyMessages, dialogId));
                }
                catch (Exception e)
                {
                    FileLog.e(e);
            ...
            if (messages != null)
            {
                for (int a = 0; a < messages.size(); a++)
                {
                    ...
                    if (did1 != 0)
                    {
                        ...
                        NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.botKeyboardDidLoaded, null, did1);
                    }
                }
            }
            else
        ...
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
        {
            try
            {
                ...
                if (botKeyboard != null)
                {
                    ...
                    AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.botKeyboardDidLoaded, botKeyboardFinal, did));
                }
            }
            catch (Exception e)
            {
        ...
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() ->
        {
            try
            {
                ...
                if (botInfo != null)
                {
                    ...
                    AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.botInfoDidLoaded, botInfoFinal, classGuid));
                }
            }
            catch (Exception e)
            {
