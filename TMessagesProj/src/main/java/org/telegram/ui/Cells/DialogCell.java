...
public class DialogCell extends BaseCell
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        int margin = AndroidUtilities.dp(AndroidUtilities.isTablet() ? 28 : 24);
        int x = LocaleController.isRTL ? (getMeasuredWidth() - margin) : (margin / 2) - 2;

        if (user != null && !(user.self || user.bot || (user.id == 333000 || user.id == 777000)) && FeaturedSettings.chatsSettings.showChatStatusIndicator)
        {
            statusBG.setStroke(AndroidUtilities.dp(2), Theme.getColor(Theme.key_windowBackgroundGray));
            setDrawableBounds(statusBG, x, AndroidUtilities.dp(46), size, size);
            statusBG.draw(canvas);
        }

        if (isFav)
        {
            setDrawableBounds(Theme.dialogs_favoriteDrawable, x - 2, 10, size, size);
